...
5. Use **Endpoint** deployment type, accept the default **GPU Configuration** and press **Create Endpoint** (Usage is billed per millisecond, but only when workers in `running` state)
...
6. After it is created, we want to get the vLLM Endpoint URL, but we will use the **OpenAI-compatible** `/v1/chat/completions` endpoint instead.
   1. Open up the requests tab for your vLLM v#.##.# endpoint and copy the `run` url.
   ...
   3. Paste the new URL into the `Settings/vLLM Endpoint` text field
...
7. Go to the **Overview** tab and generate a new API key.
   ...
   Paste your API key into the `Settings/Bearer API Key` text field.
...
8. Next, the quickest thing to do is press the `Detect Model` button. This will find the model name that you used when you setup your vLLM endpoint on [Runpod](https://get.runpod.io/q0btky2wuu29).
   ...
   Another way to find the model value used byu the worker, is check the **Logs** tab, and in the first 10 lines, you should find some text, for example `model qwen/qwen3.8-27b`. The model name is lower case compared to what we chose when we set this up at the beginning. Use the value `qwen/qwen3.8-27b` in the `Settings/Model` text field.
